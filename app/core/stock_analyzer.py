...
class StockAnalyzer:
    def __init__(self, initial_cash=1000000):
        ...
        if not self.gemini_api_key:
            self.gemini_api_key = "AIzaSyDubO89zFGF19lpgIW1CEXRIz_tKt6VAbk"
        if not self.gemini_api_url:
            self.gemini_api_url = "https://api-proxy.me/gemini"
    ...
    def get_ai_analysis(self, df, stock_code):
        ...
        try:
            ...
            response = requests.post(
                f"{self.gemini_api_url}/v1beta/models/gemini-2.0-flash:generateContent?key={self.gemini_api_key}",
                headers=headers,
                json=data,
                timeout=15
            )
